...
object MemoryModelSaver {
    ...
    fun calculateNotEmptyRanges(memoryModel: MemoryModel): List<IntRange> {
        val emptyPredicate: (Int) -> Boolean = { memoryModel.getRequiredMemoryCellByAddress(it).value == 0 }
        val notEmptyPredicate: (Int) -> Boolean = { memoryModel.getRequiredMemoryCellByAddress(it).value != 0 }
            ...
            for (range in calculateNotEmptyRanges(memoryModel)) {
                ...
                val dataSequence = range.asSequence()
                    .map { CommonUtils.intToShortPrefixedHexString(
                        memoryModel.getRequiredMemoryCellByAddress(it).value) }
